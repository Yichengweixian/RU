...
public sealed class GameManager : MonoBehaviour {
    ...
    RuntimeStageChecks qaStageChecks;
#endif
    ...
    void Start() {
        ...
        RefreshCharts();
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        if(Array.IndexOf(Environment.GetCommandLineArgs(),"--verify-stage")>=0) qaStageChecks=new RuntimeStageChecks(qaChart);
#endif
#if UNITY_EDITOR
    ...
    IEnumerator LoadAndPlay(string path) {
        ...
        if (practice) {
            ...
            chart = new ChartData { title = chart.title, artist = chart.artist, difficulty = chart.difficulty, audioName = chart.audioName,
                sourcePath = chart.sourcePath, warnings = chart.warnings, notes = included, tempo = chart.tempo, musicOffset = chart.musicOffset };
        }
        ...
        stageMessage = stage.Load(chart.sourcePath,chart);
        stage.Seek(practice ? rangeStart : double.NegativeInfinity,practice ? rangeStart-Offset : double.NegativeInfinity);
    ...
    void Update() {
...
#if UNITY_EDITOR || DEVELOPMENT_BUILD
            if(qaStageChecks!=null) qaStageChecks.Observe(stage,SongTime,true);
#endif
        ...
        stage.Tick(SongTime,JudgeTime);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        if(qaStageChecks!=null) qaStageChecks.Observe(stage,SongTime,false);
#endif
    ...
    void LateUpdate() {
        ...
        stage.PresentationVisible = show;
    ...
    void DrawPlayfield() {
        ...
        StagePreview.Draw(new Rect(930,552,290,76),stage.Animations,small);
...
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        if(qaStageChecks!=null) ok=qaStageChecks.Finish(qaDir)&&ok;
#endif
