...
public sealed class GameManager : MonoBehaviour {
    ...
    float judgeUntil;
    Vector2 scroll;
    GUIStyle normal, small, title, centered, button, big, laneText;
    Font font;
    ...
    void Update() {
        ...
        if (String.IsNullOrEmpty(qaMode)) {
            for (int lane = 0; lane < 4; lane++) {
                ...
                feedback.State.SetKey(lane, held[lane], Input.GetKeyDown(keys[lane]));
    ...
    void OnHitStarted(int note, Judgement value) {
        if (!chart.notes[note].IsHold) return;
        judge = value + " · HOLD";
    ...
    void DrawMenu() {
        ...
        GUI.Label(new Rect(735, 224, 460, 35), "下落速度  " + speed.ToString("F0"), small);
        Fill(new Rect(737, 272, 400, 3), .25f);
        speed = GUI.HorizontalSlider(new Rect(737, 265, 400, 25), speed, 200, 520);
        GUI.Label(new Rect(735, 297, 460, 32), "对音偏移  " + offsetMs + " ms", small);
        if (GUI.Button(new Rect(735, 337, 110, 38), "− 10", button)) offsetMs = Math.Max(-200, offsetMs - 10);
        if (GUI.Button(new Rect(863, 337, 110, 38), "+ 10", button)) offsetMs = Math.Min(200, offsetMs + 10);
        if (GUI.Button(new Rect(991, 337, 145, 38), "重置", button)) offsetMs = 0;
        GUI.Label(new Rect(735, 386, 440, 42), "正值：音符比音乐更晚到达判定线。", small);
        GUI.Label(new Rect(735, 431, 440, 30), "打击音量  " + Mathf.RoundToInt(feedback.Volume*100) + "%", small);
        Fill(new Rect(737, 480, 400, 3), .25f);
        feedback.Volume = GUI.HorizontalSlider(new Rect(737, 473, 400, 22), feedback.Volume, 0, 1);
    ...
    void SaveSettings() { PlayerPrefs.SetInt("RhythmOffsetMs", offsetMs); PlayerPrefs.SetFloat("RhythmSpeed", speed); PlayerPrefs.SetFloat("RhythmHitVolume", feedback.Volume); PlayerPrefs.Save(); }
    void DrawPlayfield() {
        ...
        for (int i = 0; i < 4; i++) {
            feedback.DrawBase(i, i*LaneWidth, LaneWidth, localJudgeY);
        ...
        for (int i = 0; i < 4; i++) feedback.DrawEffects(i, i*LaneWidth, LaneWidth, localJudgeY, JudgeTime, laneText);
